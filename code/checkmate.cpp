            ...
            if(pieces[i][j].color == 1){
               if(i < 7){ //Do not pass the board!
                    if(i + 1 < 8 && j + 1 < 8){ //test if the sides are inside the board
                        if(pieces[i+1][j+1].type != BLANK && pieces[i+1][j+1].color != pieces[i][j].color){ //if the diagonal squares are from a different color
                            pieces[i][j].attackBoard[i+1][j+1] = 1;
                        }
                        if(j - 1 >= 0){ //same here
                            if(pieces[i+1][j-1].type != BLANK && pieces[i+1][j-1].color != pieces[i][j].color){
                                pieces[i][j].attackBoard[i+1][j-1] = 1;
                            }
                        }
                    }
...
void CheckMate::updateTower(Piece pieces[8][8], int i, int j){
    bool stop = false;
    int k;
    if(i < 7){ //front
...
void CheckMate::updateBishop(Piece pieces[8][8], int i, int j){
    int k, p;
    bool stop = false;
    if(i < 7){ //front
        if(j < 7){ //front-right
...
void CheckMate::updateKnight(Piece pieces[8][8], int i, int j){
...
void CheckMate::updateKing(Piece pieces[8][8], int i, int j){
...
void CheckMate::updateAttackBoard(Piece board[8][8]){
    int i, j;
...
bool CheckMate::testKingMove(Piece pieces[8][8], int iKing, int jKing){
    int squares = 8, i, j;
...
bool CheckMate::testBlockQueen(Piece pieces[8][8], int iQueen, int jQueen, int iKing, int jKing){
...
bool CheckMate::testBlockTower(Piece pieces[8][8], int iTower, int jTower, int iKing, int jKing){
    int i, j, k;
    int color = pieces[iKing][jKing].color;
    if(iTower == iKing){
        if(jTower < jKing){
...
bool CheckMate::testBlockBishop(Piece pieces[8][8], int iBishop, int jBishop, int iKing, int jKing){
    int i, j, k, p;
    int color = pieces[iKing][jKing].color;
    if(iKing > iBishop){
        if(jKing > jBishop){
...
bool CheckMate::testBlockMove(Piece pieces[8][8], int iKing, int jKing){
    int i, j;
    int color = pieces[iKing][jKing].color;
    for(i = 0; i < 8; i++){
        for(j = 0; j < 8; j++){
...
bool CheckMate::testCaptureMove(Piece pieces[8][8], int iKing, int jKing){
    int i, j;
...
bool CheckMate::testCapture(Piece pieces[8][8], int iX, int jX){
    int i, j;
